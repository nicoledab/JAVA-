...
<%
  ...
  /*
   ...
   // int number=0;      //화면의 글 번호(DB의 글번호가 아님)
    ...
    count = dbPro.getreviewCount(col, search);
    if (count > 0) {        
    	reviewList = dbPro.getreview(col, search);
    }
    ...
    <%-- dto는 하나의 db 정보만...
     dto를 모두 보관하는 array list--%> 
       ...
       <form action="rsearchList.jsp" method="post">
  		 <select name="col"> 
            <option name="subject">가격대 선택<option> 
            <option name="subject">선택 안함<option> 
            <option name="price2">2만원 이하<option> 
            <option name="price35">3~5만원<option> 
            <option name="price5">5만원 이상<option> 
   		</select>
