...
<table width="500" border="1" cellspacing="0" cellpadding="0"  bgcolor="<%=bodyback_c%>" align="center">  
  ...
  <tr height="30">      
		...
		if(id != null) { 
			...
			<input type="button" value="글수정" onclick="window.location='updateForm2.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
			<input type="button" value="글삭제" onclick="window.location='deleteForm2.jsp?num=<%=article.getNum()%>&pageNum=<%=pageNum%>'">
		   		<%}%>
		<input type="button" value="답글쓰기" onclick="window.location='writeFormNew.jsp?num=<%=num%>&ref=<%=ref%>&re_step=<%=re_step%>&re_level=<%=re_level%>'">
		<%}%>
			
			<input type="button" value="글목록" onclick="window.location='Mlist.jsp?pageNum=<%=pageNum%>'">
	    </td>
  </tr>
</table>
</body>
