...
<table width="700">
	<tr>
    	...
    	<%if(id != null){ %>
    		<a href="writeFormNew.jsp">글쓰기</a> / 
    		 <a href="myListNew.jsp">나의 작성글 목록</a> /
    		  <a href="myListNew.jsp">나의 작성글 목록</a> /
    		  <a href="/WebPractice/member/Mlogout.jsp" onsubmit="return logout();" >로그아웃</a>
    		<%}else{ %>
    		<a href="/WebPractice/member/login.jsp">로그인 후 글쓰기</a>
    		<%} %>
    	</td>
    </tr>
</table>
	...
	<tr height="30">
    	...
    	<td  width="250" >
			...
			<%}else{%>
		  		<img src="images/level.gif" width="<%=wid%>" height="16">
			<%}%>
     		 <a href="contentNew.jsp?num=<%=article.getNum()%>&pageNum=<%=currentPage%>">
           		<%=article.getSubject()%>
           	 </a> 
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="Mlist.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        	<a href="Mlist.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%		}
        if (endPage < pageCount) {  %>
        	<a href="Mlist.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%		}
    }
%>

   <form action="searchList2.jsp" method="post">
  		 <select name="col"> 
